...
void ChangeTitleMode() {
    ...
    strcat(title, (data->pencilMarkMode) ? " (Pencil Mark)" : ""); // rajoute (Pencil Mark) si on est en mode pencil mark
    SDL_SetWindowTitle(data->window, title); // met à jour le titre
}